...
function passesSetup(stock, setupNum) {
  ...
  if (setupNum !== 3) {
    // P/L máximo (100 = sem limite, 0 = sem limite também)
    if (s.pl > 0 && s.pl < 100 && pl && pl > s.pl) return false;

    // P/VP máximo (20 = sem limite, 0 = sem limite também)
    if (s.pvp > 0 && s.pvp < 20 && pvp) {
...
function updateSummary(n) {
  ...
  if (n !== 3) {
    tags.push(`P/L: ${s.pl >= 100 || s.pl === 0 ? 'qualquer' : 'até ' + s.pl}`);
    tags.push(`P/VP: ${s.pvp >= 20 || s.pvp === 0 ? 'qualquer' : 'até ' + (s.pvp / 2).toFixed(1)}`);
  }
