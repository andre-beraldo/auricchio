    ...
    return `
      <div class="nivel-row ${classes[Math.min(i, 2)]}" data-index="${i}">
        ...
        <input class="nivel-pct-input" type="number" min="0" max="99" step="0.1"
          value="${isAuto ? (levels[i-1]?.from ?? 0) : lvl.from}"
          ${isAuto ? '' : ''}
...
function updateFrom(index, value, isAuto) {
  ...
  if (!isNaN(num)) {
    if (isAuto) {
      // O forte não tem from próprio — o valor é calculado do anterior
      // Mas agora deixamos o usuário editar o piso do forte
      tempConfig.levels[index].from = num;
      delete tempConfig.levels[index].auto;
      tempConfig.levels[index].auto = true; // mantém como auto mas usa o valor
    } else {
      tempConfig.levels[index].from = num;
    }
  }
...
function applyFilters(stockList, searchTerm = '') {
  ...
  // Teto de 25% — não se aplica ao filtro forte (decisão consciente do usuário)
  if (currentDiscountFilter !== 'strong') {
