...
function abrirModalGrafico(ticker) {
  ...
  _modalState.ticker = stock.ticker;
  _modalState.preco  = stock.price;
  _modalState.media  = stock.target;
  _modalState.anos   = discountConfig.periodo;
  ...
  // Reseta botões de período para o período atual
  document.querySelectorAll('#modalPeriodoRow .gpbtn').forEach((b, i) => {
    b.classList.toggle('sel', i + 1 === _modalState.anos);
  });
  ...
  // Aguarda render do canvas antes de desenhar
  setTimeout(() => _desenharGraficoModal(_modalState.anos, stock.price, stock.target), 150);
}

function fecharModalGrafico(event) {
  // Se chamado sem argumento (botão ✕): fecha direto
  // Se chamado pelo overlay: só fecha se clicou no próprio overlay (não no box)
...
function _fecharModal() {
  ...
  if (_modalState.raf) {
    cancelAnimationFrame(_modalState.raf);
    _modalState.raf = null;
  }
}

function setModalPeriodo(anos, el) {
  _modalState.anos = anos;
  document.querySelectorAll('#modalPeriodoRow .gpbtn').forEach(b => b.classList.remove('sel'));
  el.classList.add('sel');
  _desenharGraficoModal(anos, _modalState.preco, _modalState.media);
}

/* helper: arredondamento de canto no canvas */
...
function _desenharGraficoModal(anos, preco, media) {
  if (_modalState.raf) cancelAnimationFrame(_modalState.raf);
  ...
  function draw() {
    ...
    ctx.fillText(`Média  ${formatBRL(media)}`, 8, mY - 6);
    ...
    /* ponto animado + tooltip "Hoje" quando animação termina */
    if (prog >= 1) {
      const lx    = toX(dados.length - 1);
      const ly    = toY(dados[dados.length - 1]);
      const pulse = Math.sin(Date.now() / 280) * 2;

      ctx.beginPath(); ctx.arc(lx, ly, 4, 0, Math.PI * 2);
      ctx.fillStyle = '#3b82f6'; ctx.fill();
      ctx.beginPath(); ctx.arc(lx, ly, 7 + pulse, 0, Math.PI * 2);
      ctx.strokeStyle = 'rgba(59,130,246,0.3)'; ctx.lineWidth = 1.5; ctx.stroke();

      /* tooltip fixo "Hoje" */
      const ttW = 92, ttH = 38;
      const ttX = Math.max(4, Math.min(lx - ttW / 2, W - ttW - 4));
      const ttY = Math.max(4, ly - ttH - 12);

      ctx.fillStyle   = 'rgba(15,23,42,0.94)';
      ctx.strokeStyle = 'rgba(59,130,246,0.35)';
      ctx.lineWidth   = 1;
      _roundRect(ctx, ttX, ttY, ttW, ttH, 6);
      ctx.fill(); ctx.stroke();

      const precoStr = formatBRL(preco);
      ctx.fillStyle = '#3b82f6';
      ctx.font      = 'bold 11px -apple-system, sans-serif';
      ctx.fillText(precoStr, ttX + (ttW - ctx.measureText(precoStr).width) / 2, ttY + 15);

      ctx.fillStyle = '#64748b';
      ctx.font      = '9px -apple-system, sans-serif';
      ctx.fillText('Hoje', ttX + (ttW - ctx.measureText('Hoje').width) / 2, ttY + 28);
    }

    prog = Math.min(1, prog + 0.028);
    _modalState.raf = requestAnimationFrame(draw);
  }
  ...
  canvas.onmousemove = (e) => {
    if (!_modalState.dados || prog < 0.5) return;
    const rect    = canvas.getBoundingClientRect();
    const mx      = e.clientX - rect.left;
    const idx     = Math.round((mx / rect.width) * (_modalState.dados.length - 1));
    const safeIdx = Math.min(Math.max(idx, 0), _modalState.dados.length - 1);
    const val     = _modalState.dados[safeIdx];

    const mesesAtras = _modalState.dados.length - 1 - safeIdx;
    const dataStr    = mesesAtras === 0 ? 'Hoje' : `há ${mesesAtras}m`;

    document.getElementById('modalTooltipVal').textContent  = formatBRL(val);
    document.getElementById('modalTooltipData').textContent = dataStr;
    document.getElementById('modalTooltip').classList.add('visible');
  };

  canvas.onmouseleave = () => {
    document.getElementById('modalTooltip').classList.remove('visible');
  };

  /* touch support para mobile */
  canvas.ontouchmove = (e) => {
    e.preventDefault();
    if (!_modalState.dados) return;
    const rect    = canvas.getBoundingClientRect();
    const tx      = e.touches[0].clientX - rect.left;
    const idx     = Math.round((tx / rect.width) * (_modalState.dados.length - 1));
    const safeIdx = Math.min(Math.max(idx, 0), _modalState.dados.length - 1);
    const val     = _modalState.dados[safeIdx];
    const mesesAtras = _modalState.dados.length - 1 - safeIdx;

    document.getElementById('modalTooltipVal').textContent  = formatBRL(val);
    document.getElementById('modalTooltipData').textContent = mesesAtras === 0 ? 'Hoje' : `há ${mesesAtras}m`;
    document.getElementById('modalTooltip').classList.add('visible');
  };
