...
const CONFIG = {
  ...
  REQUEST_DELAY: 400,
  BATCH_SIZE:    25   // 50 ações ÷ 25 = apenas 2 requisições
};
...
/* ================= AÇÕES (50 CURADAS — BLUE CHIPS POR SETOR) ================= */
// Critério de seleção: maior liquidez e representatividade por setor.
// 50 ações ÷ BATCH_SIZE 25 = 2 requisições à BRAPI (era 82 com 164 ações).
const stocks = [
  // ========== BANCOS (5) ==========
  ...
  // ========== LOGÍSTICA E CONCESSÕES (2) ==========
  { ticker: 'RAIL3',  name: 'Rumo',                sector: 'Logística',           category: 'ciclica' },
  { ticker: 'ECOR3',  name: 'Ecorodovias',         sector: 'Concessões',          category: 'perene'  },
...
/**
 * Busca todos os 50 tickers em lotes de 25 → apenas 2 requisições à BRAPI.
 * Antes: 164 ações ÷ 2 = 82 requisições.
 * Agora:  50 ações ÷ 25 = 2 requisições.
 */
async function fetchAllStocks() {
  ...
  for (let i = 0; i < stocks.length; i += CONFIG.BATCH_SIZE) {
    ...
    try {
      ...
      // Delay apenas entre lotes intermediários (sem delay desnecessário no último)
