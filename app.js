...
(async function autoCleanup() {
  ...
  if (lastCleanup !== SW_VERSION) {
    ...
    try {
      ...
      const keysToKeep = ['stocksCache_v3'];  // v2_clean removido — era o cache corrompido
...
const CONFIG = {
  ...
  CACHE_KEY:     'stocksCache_v3',
  CACHE_TIME:    4 * 60 * 60 * 1000,   // 4 horas
  REQUEST_DELAY: 400,
  BATCH_SIZE:    25                      // 50 ações ÷ 25 = 2 requisições (Yahoo Finance não tem limite)
};
...
async function loadApp() {
  ...
  // Salva no cache para as próximas 4 horas
