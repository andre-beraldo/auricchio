...
function getBadgeText(discount) {
  ...
  // Percorre do maior nível manual para o menor
  const manual = getManualLevels();
  for (let i = manual.length - 1; i >= 0; i--) {
    if (discount >= manual[i].from) return manual[i].name;
  }
  // Abaixo do primeiro threshold — ainda assim usa o nome do primeiro nível
...
function applyConfigLabels() {
  ...
  // Nível 1 → moderate (só se existir)
  ...
  if (temMod) {
    ...
    // Se estava no filtro moderado e foi removido, volta para light
    ...
    return `
      <div class="nivel-row ${classes[Math.min(i, 2)]}" data-index="${i}">
        <div class="nivel-top">
          ...
          <span class="nivel-tag">${isAuto ? `Nível ${i + 1} · automático` : `Nível ${i + 1}`}</span>
...
function removerNivel(index) {
  ...
  if (manual.length <= 1) return; // mínimo 1 nível manual
...
function salvarConfig() {
  ...
  // Re-renderiza com os novos critérios — garante que as ações aparecem
  closeConfig();
...
function applyFilters(stockList, searchTerm = '') {
  ...
  let filtered = stockList.filter(stock => {
    ...
    if (currentDiscountFilter === 'strong') {
      // Forte = acima do último threshold manual
      const lastFrom = manual[manual.length - 1]?.from ?? l0from;
      return d >= lastFrom;
    }

    if (currentDiscountFilter === 'moderate') {
      if (!temMod) return false; // sem nível moderado
  ...
  if (discountConfig.plMax !== null && discountConfig.plMax !== undefined) {
    ...
    if (!isNaN(maxPL) && maxPL > 0) {
      filtered = filtered.filter(stock => {
        ...
        if (!pl || pl <= 0) return false; // exclui N/A e negativo
        return pl <= maxPL;
      });
    }
  }
