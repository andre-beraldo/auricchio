...
async function renderStocks(stockList) {
  ...
  // Busca histórico de todas as ações em paralelo (mais rápido)
  const histories = await Promise.all(
    stockList.map(stock => fetchPriceHistory(stock.ticker))
  );
  ...
  app.innerHTML = stockList.map((stock, index) => {
    ...
    const { items, performanceItems } = analyzeHealth(stock.fundamentals, histories[index]);
...
async function loadApp(searchTerm = '') {
  ...
  await renderStocks(filteredStocks);
