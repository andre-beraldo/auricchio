...
let currentDiscountFilter = null; // Adiciona essa variável global
...
async function loadApp(filter = '') {
  ...
  // ADICIONA O FILTRO DE DESCONTO AQUI
  if (currentDiscountFilter) {
    filtered = filtered.filter(s => {
      if (currentDiscountFilter === 'strong') return s.discount >= 16.6;
      if (currentDiscountFilter === 'moderate') return s.discount >= 8.3 && s.discount < 16.6;
      if (currentDiscountFilter === 'light') return s.discount > 0 && s.discount < 8.3;
      return true;
    });
  }
...
// ADICIONA OS EVENT LISTENERS DOS BOTÕES
document.querySelectorAll('.filter-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.filter-btn').forEach(b => b.classList.remove('active'));
    btn.classList.add('active');
    
    currentDiscountFilter = btn.dataset.filter;
    loadApp(document.getElementById('search').value.toUpperCase());
  });
});
