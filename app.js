...
function loadDiscountConfig() {
  try {
    ...
    // Migra configs antigas que tinham plMax global
...
// Retorna o nível correspondente ao desconto atual
function getLevelForDiscount(discount) {
  const manual = getManualLevels();
  const levels = discountConfig.levels;
  ...
  if (currentDiscountFilter === 'strong') {
    return levels[levels.length - 1]; // nível forte/auto
  }
  if (currentDiscountFilter === 'moderate' && temMod) {
    return manual[1];
  }
  return manual[0];
}
...
function toggleMenu() { console.log('menu'); }
...
function openTop(tipo) {
  closeMenu();
  // lógica dos TOPs será implementada na próxima etapa
  console.log('TOP:', tipo);
}
...
/* ── ESTADO DO MODO TOP ── */
let modoTop = null; // null | 'dividendos' | 'patrimonial' | 'valuation'
 
/* ── DADOS SIMULADOS POR AÇÃO (substituir pela BRAPI Pro) ── */
const TOPS_DATA = {
...
function closeTopMode() {
  ...
  document.getElementById('btnVoltarPadrao').style.display = 'none';
...
function renderTopMode() {
  ...
  if (!banner) {
    ...
    banner.id = 'topBanner';
    banner.className = 'top-mode-banner';
    banner.innerHTML = `<span id="topBannerLabel"></span>`;
    const btnPersonalizar = document.querySelector('.btn-personalizar');
    btnPersonalizar.parentNode.insertBefore(banner, btnPersonalizar);
  }
  banner.className = `top-mode-banner ${cfg.cls} visible`;
  document.getElementById('topBannerLabel').textContent = cfg.label;

  document.getElementById('btnVoltarPadrao').style.display = 'block';
 
  // Atualiza stat card "Total de Ações"
  const totalLabel = document.querySelector('.stat-card .stat-label');
  const totalValue = document.getElementById('totalStocks');
  if (totalLabel) totalLabel.textContent = cfg.label.replace(/.*? /, '');
  const cores = { div: '#10b981', pat: '#a78bfa', val: '#f59e0b' };
  if (totalValue) totalValue.style.color = cores[cfg.cls];
 
  // Filtra só ações em desconto, ordena pelo critério
