...
async function fetchAllStocks() {
  ...
  for (let i = 0; i < stocks.length; i += CONFIG.BATCH_SIZE) {
    ...
    try {
      ...
      console.warn(`⚠️ Erro lote ${i}-${i + CONFIG.BATCH_SIZE}:`, error.message);
    }
  }
  return data;
}
...
async function renderStocks(stockList) {
    ...
    return `
      <div class="stock-card ${borderClass}" data-index="${index}">
        <div class="stock-header">
          ...
          <div class="discount-badge ${discountLevel}">${badgeText}</div>
        </div>
  ...
  // Carrega performance em segundo plano
...
function applyFilters(stockList, searchTerm = '') {
  ...
  if (searchTerm) {
    ...
    filtered = filtered.filter(stock => {
      ...
      const name    = stock.name.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      const sector  = stock.sector.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      const aliases = (stock.aliases || []).join(' ').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
      return ticker.includes(normalizedSearch) || name.includes(normalizedSearch) ||
             sector.includes(normalizedSearch)  || aliases.includes(normalizedSearch);
    });
  ...
  if (currentSectorFilter && currentSectorFilter !== 'Todos') {
    filtered = filtered.filter(stock => stock.sector === currentSectorFilter);
  }
...
/* ================= FILTRO POR SETOR ================= */
function getSectorsForDiscount(discountLevel, stockList) {
  const filtered = stockList.filter(stock => {
    if (discountLevel === 'strong')   return stock.discount >= 16.6;
    if (discountLevel === 'moderate') return stock.discount >= 8.3 && stock.discount < 16.6;
    if (discountLevel === 'light')    return stock.discount > 0 && stock.discount < 8.3;
    return true;
  });
...
function toggleSectorDropdown(discountLevel) {
  document.querySelectorAll('.sector-dropdown').forEach(d => {
    if (d.id !== `dropdown-${discountLevel}`) d.classList.remove('open');
  });

  const dropdown = document.getElementById(`dropdown-${discountLevel}`);
  if (!dropdown) return;

  const sectors = getSectorsForDiscount(discountLevel, allProcessedStocks);
  dropdown.innerHTML = sectors.map(sector => `
    <div class="sector-option ${sector === currentSectorFilter ? 'selected' : ''}"
         onclick="selectSector('${discountLevel}', '${sector}')">
      ${sector}
    </div>
  `).join('');
...
function selectSector(discountLevel, sector) {
  currentDiscountFilter = discountLevel;
  currentSectorFilter   = sector;

  document.querySelectorAll('.filter-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.filter === discountLevel);
    const subEl = btn.querySelector('.filter-btn-sector');
    if (subEl) subEl.remove();
    if (btn.dataset.filter === discountLevel && sector !== 'Todos') {
      const sub = document.createElement('span');
      sub.className = 'filter-btn-sector';
      sub.textContent = sector;
      btn.appendChild(sub);
    }
  });

  document.getElementById(`dropdown-${discountLevel}`)?.classList.remove('open');

  const filtered = applyFilters(allProcessedStocks, searchInput.value.toUpperCase());
...
function updateStatistics(allStocks) {
  ...
  if (cacheData) {
    ...
    document.getElementById('cacheTime').textContent = hours > 0 ? `${hours}h` : `${minutes}min`;
  } else {
...
async function loadApp(searchTerm = '') {
  ...
  const processedStocks = stocks.map(stock => {
    ...
    if (!apiStock.regularMarketPrice || apiStock.regularMarketPrice <= 0) {
      console.warn(`⚠️ ${stock.ticker}: preço inválido`);
      return null;
    }
    if (!apiStock.fiftyTwoWeekLow || !apiStock.fiftyTwoWeekHigh ||
        apiStock.fiftyTwoWeekHigh <= apiStock.fiftyTwoWeekLow) {
      console.warn(`⚠️ ${stock.ticker}: dados históricos inválidos`);
      return null;
    }

    const target   = targetFromHistory(apiStock.fiftyTwoWeekLow, apiStock.fiftyTwoWeekHigh, stock.category);
  ...
  console.log('📊 RESUMO:');
  console.log(`Total no array: ${stocks.length}`);
...
document.querySelectorAll('.filter-btn').forEach(button => {
  button.addEventListener('click', () => {
    ...
    document.querySelectorAll('.filter-btn-sector').forEach(el => el.remove());
    document.querySelectorAll('.sector-dropdown').forEach(d => d.classList.remove('open'));

    loadApp(searchInput.value.toUpperCase());
  });
});
