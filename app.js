...
/* ── UTILITÁRIOS DE DESCONTO (baseados no config) ── */
function getDiscountLevel(discount) {
  const levels = discountConfig.levels;
  // Percorre de trás pra frente (nível mais alto primeiro)
  for (let i = levels.length - 2; i >= 0; i--) {
    if (discount >= levels[i].from) {
      if (i === 0) return 'light';
...
function getBadgeText(discount) {
  const levels = discountConfig.levels;
  for (let i = levels.length - 2; i >= 0; i--) {
    if (discount >= levels[i].from) return levels[i].name;
  }
  // auto (último nível)
...
function getBadgeClass(discount) {
  const levels = discountConfig.levels;
  for (let i = levels.length - 2; i >= 0; i--) {
    if (discount >= levels[i].from) return getLevelKey(i);
  }
  return getLevelKey(levels.length - 1);
}
...
/* ── ATUALIZA LABELS DO HTML COM CONFIG ── */
function applyConfigLabels() {
  const levels = discountConfig.levels;
  const keys   = ['light', 'moderate', 'strong'];

  levels.forEach((lvl, i) => {
    const key = keys[i];
    const btnLabel  = document.getElementById(`btn-label-${key}`);
    const statLabel = document.getElementById(`stat-label-${key}`);
    const name = lvl.auto ? levels[i - 1] ? levels[i - 1].name.replace('Leve','Forte').replace('Moderado','Forte') : lvl.name : lvl.name;
    const displayName = lvl.name;
    if (btnLabel)  btnLabel.textContent  = displayName.length > 12 ? displayName.substring(0, 12) + '…' : displayName;
    if (statLabel) statLabel.textContent = displayName;
  });
}
...
function renderConfigScreen() {
  ...
  const tagLabels = ['Nível 1', 'Nível 2', 'Nível 3 · automático'];

  container.innerHTML = levels.map((lvl, i) => {
    const isAuto  = !!lvl.auto;
    const canRemove = !isAuto && levels.length > 2;

    return `
      <div class="nivel-row ${classes[i]}" data-index="${i}">
        <div class="nivel-top">
          <div class="nivel-dot"></div>
          <span class="nivel-tag">${tagLabels[i]}</span>
          ${canRemove ? `<button class="btn-remover" onclick="removerNivel(${i})">✕ remover</button>` : ''}
        </div>
        <div class="nivel-fields">
          <div class="field-group">
            <label>Nome</label>
            <input class="field-input" type="text" value="${lvl.name}"
...
function updateFrom(index, value) {
  ...
  if (!isNaN(num)) {
    ...
    // Atualiza o campo "acima de X%" do nível automático
...
function removerNivel(index) {
  if (tempConfig.levels.length <= 2) return; // mínimo 2
  ...
  if (currentDiscountFilter) {
    filtered = filtered.filter(stock => {
      ...
      if (currentDiscountFilter === 'strong') {
        // Último nível manual (antes do auto)
        const lastManual = levels.filter(l => !l.auto).pop();
        return d >= (lastManual?.from ?? 16.6);
      }
      if (currentDiscountFilter === 'moderate' && levels.length >= 3) {
        const l1 = levels[0].from ?? 8.3;
        const l2 = levels[1].from ?? 16.6;
        return d >= l1 && d < l2;
      }
...
function updateStatistics(allStocks) {
  const levels = discountConfig.levels;
  const l1from = levels[0]?.from ?? 8.3;
  const l2from = levels[1]?.from ?? 16.6;

  const s = {
    total:    allStocks.length,
    strong:   allStocks.filter(s => s.discount >= l2from).length,
    moderate: allStocks.filter(s => s.discount >= l1from && s.discount < l2from).length,
    light:    allStocks.filter(s => s.discount > 0 && s.discount < l1from).length
  };
