...
async function fetchStocks() {
  ...
  console.log('📡 Buscando dados da BRAPI em lotes...');
  let allResults = [];

  for (let i = 0; i < stocks.length; i += BATCH_SIZE) {
    const batch = stocks.slice(i, i + BATCH_SIZE);
    const url = `https://brapi.dev/api/quote/${batch.join(',')}?token=${BRAPI_API_KEY}`;
    ...
    try {
      ...
      console.log(`✔️ Lote ${i / BATCH_SIZE + 1} carregado`);
...
searchInput.addEventListener('input', e => {
  ...
  render(window.stockResults.filter(s => s.ticker.toLowerCase().includes(term)));
});
