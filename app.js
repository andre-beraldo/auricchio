...
const CONFIG = {
  ...
  CACHE_TIME:    12 * 60 * 60 * 1000,   // 12 horas
  REQUEST_DELAY: 1000,               // 1s entre requisições diretas ao BRAPI do browser
...
/* ================= API ================= */
// Chave BRAPI — uso pessoal, pode regenerar em brapi.dev se necessário
const BRAPI_TOKEN = '4GPeeg5frU4MAtdp44bEqZ';

async function fetchDataFromAPI(tickers) {
  try {
    // Chamada direta ao BRAPI do browser — evita bloqueio por IP do Cloudflare
    const url = `https://brapi.dev/api/quote/${tickers}?token=${BRAPI_TOKEN}&fundamental=true&modules=defaultKeyStatistics`;
