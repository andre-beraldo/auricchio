...
/* ── DADOS SIMULADOS TOPs (substituir pela BRAPI Pro) ── */
const TOPS_DATA = {
  'ITUB4':  { dy: 6.2,  pvp: 1.42, pl: 8.1  },
  'BBAS3':  { dy: 8.8,  pvp: 0.92, pl: 4.1  },
  'BBDC4':  { dy: 5.4,  pvp: 0.88, pl: 7.2  },
  'TAEE11': { dy: 12.4, pvp: 1.10, pl: 8.2  },
  'EGIE3':  { dy: 8.1,  pvp: 2.20, pl: 10.4 },
  'CMIG4':  { dy: 9.6,  pvp: 1.05, pl: 6.2  },
  'PETR4':  { dy: 14.2, pvp: 1.80, pl: 3.8  },
  'VALE3':  { dy: 7.8,  pvp: 1.60, pl: 5.2  },
  'RADL3':  { dy: 1.2,  pvp: 4.10, pl: 28.4 },
  'FLRY3':  { dy: 3.4,  pvp: 2.40, pl: 18.6 },
  'LREN3':  { dy: 4.8,  pvp: 2.10, pl: 14.2 },
  'ASAI3':  { dy: 2.1,  pvp: 3.20, pl: 24.9 },
  'WEGE3':  { dy: 1.8,  pvp: 6.40, pl: 32.1 },
  'GGBR4':  { dy: 5.6,  pvp: 0.88, pl: 6.4  },
  'BBSE3':  { dy: 9.8,  pvp: 3.10, pl: 9.2  },
  'VIVT3':  { dy: 6.4,  pvp: 1.90, pl: 11.8 },
  'SBSP3':  { dy: 3.2,  pvp: 1.40, pl: 16.2 },
  'KLBN11': { dy: 4.1,  pvp: 1.20, pl: 12.4 },
  'MULT3':  { dy: 3.8,  pvp: 2.80, pl: 20.1 },
  'ITSA4':  { dy: 5.9,  pvp: 1.60, pl: 9.8  },
};
...
function getTopsData(stock, campo) {
  return TOPS_DATA[stock.ticker]?.[campo] ?? 999;
}
    ...
    return `
      <div class="nivel-row ${classes[Math.min(i, 2)]}" data-index="${i}">
        <div class="nivel-row-1">
          ...
          <input class="nivel-nome-input" type="text" value="${lvl.name}" maxlength="15"
            oninput="tempConfig.levels[${i}].name = this.value">
          <span class="nivel-ate-label">${ateLabel}</span>
          <input class="nivel-pct-input" type="number" min="0" max="99" step="0.1"
            value="${pctValue}"
            oninput="updateFrom(${i}, this.value, ${isAuto})">
        ...
        <div class="nivel-row-2">
          ...
          <input class="nivel-pl-input" type="number" min="0" max="999" step="1"
            value="${plVal}" placeholder="—"
            oninput="tempConfig.levels[${i}].plMax = this.value === '' ? null : parseFloat(this.value)">
...
function updateFrom(index, value, isAuto) {
  const num = parseFloat(value);
  if (!isNaN(num)) tempConfig.levels[index].from = num;
}
...
let sobreAberto     = false;
let favAberto       = false;
let sobreTypingDone = false;
...
function toggleSobre() {
  ...
  const exp   = document.getElementById('sobreExpandable');
  const arrow = document.getElementById('sobreArrow');
  const item  = document.getElementById('sobreItem');
  exp.classList.toggle('open', sobreAberto);
  arrow.classList.toggle('open', sobreAberto);
  item.classList.toggle('active', sobreAberto);
  if (sobreAberto && !sobreTypingDone) {
    typewriterEffect('sobreText', SOBRE_TEXTO, 22);
    sobreTypingDone = true;
  }
}
...
function toggleFavoritas() {
  favAberto = !favAberto;
  const exp   = document.getElementById('favExpandable');
  const arrow = document.getElementById('favArrow');
  const item  = document.getElementById('favItem');
  exp.classList.toggle('open', favAberto);
  arrow.classList.toggle('open', favAberto);
  item.classList.toggle('active', favAberto);
...
function renderFavoritas() {
  ...
  if (favs.length === 0) {
    content.innerHTML = '<div class="fav-empty">Nenhuma ação favoritada ainda.</div>';
    return;
  }
  content.innerHTML = favs.map(fav => {
    ...
    return `
      <div class="fav-item" onclick="closeMenu()">
        <div class="fav-dot" style="background:${color}"></div>
        <span class="fav-ticker">${fav.ticker}</span>
        <span class="fav-name">${fav.name || ''}</span>
        <span class="fav-level" style="background:${color}22;color:${color};">${levelName}</span>
      </div>
    `;
  }).join('');
}
...
function getFavoritas() {
  try {
    return JSON.parse(localStorage.getItem('auricchio_favoritas') || '[]');
  } catch { return []; }
}
...
window.toggleFavoritas = toggleFavoritas;
...
/* ── PATCH 1: activateFilterBtn não mata mais o modoTop ── */
...
async function renderStocks(stockList) {
  ...
  app.innerHTML = stockList.map((stock, index) => {
    ...
    return `
      ...
      <div class="grafico-wrap" onclick="abrirModalGrafico(${index})" style="cursor:pointer;">
          <canvas id="grafico-canvas-${index}"></canvas>
          <div class="hint-fullscreen">⛶ ampliar</div>
        </div>
        ...
        <div class="grafico-periodo">
          ${[1,2,3,4,5].map(a => `
            <button class="gpbtn${a === discountConfig.periodo ? ' sel' : ''}"
              onclick="setGraficoPeriodo(${index}, ${a}, this, ${stock.price}, ${stock.target})">
              ${a}a
            </button>`).join('')}
          </div>
        </div>
...
function renderTopCards(stockList, tipo, cls) {
  if (stockList.length === 0) {
    app.innerHTML = `
      <div class="empty-state">
        <div class="empty-icon">🔍</div>
        <h3>Nenhuma ação encontrada</h3>
        <p>Tente ajustar os filtros de desconto</p>
      </div>`;
    return;
  }
  ...
  const badgeColors = {
    div: { bg: 'rgba(16,185,129,0.2)',  color: '#10b981' },
    pat: { bg: 'rgba(139,92,246,0.2)',  color: '#a78bfa' },
    val: { bg: 'rgba(245,158,11,0.2)',  color: '#f59e0b' },
  };
  ...
  app.innerHTML = stockList.map((stock, index) => {
    ...
    const manual    = getManualLevels();
    const l0        = manual[0]?.from ?? 8.3;
    const l1        = manual[1]?.from ?? 16.6;
    const badgeText = getBadgeText(discount);

    let nivelCls = 'leve';
    if (discount >= l1) nivelCls = 'forte';
    else if (discount >= l0) nivelCls = 'mod';
    ...
    return `
      <div class="stock-card ${borderCls}" data-index="${index}">
        ...
        <div class="top-highlight ${cls}">
          <div>
            <div class="top-highlight-label">${cfg.label}</div>
            <div class="top-highlight-sub">${cfg.sub}</div>
          </div>
        ...
        <div class="stock-info">
          ...
          <div class="info-row">
            <span class="info-label">Desconto</span>
            <span class="info-value" style="color:${getDiscountColor(discount)}">${discount.toFixed(1)}%</span>
          </div>
        </div>
...
function closeTopMode() {
  ...
  const totalCard  = document.getElementById('totalStocks');
  const totalLabel = totalCard?.previousElementSibling;
...
/* ── PATCH 2 + 3: renderTopMode respeita currentDiscountFilter ── */
function renderTopMode() {
  const configs = {
    dividendos:  { label: '💰 TOPs Dividendos',  cls: 'div', sort: (a,b) => getTopsData(b,'dy')  - getTopsData(a,'dy'),  campo: 'dy'  },
    patrimonial: { label: '🏛️ TOPs Patrimonial', cls: 'pat', sort: (a,b) => getTopsData(a,'pvp') - getTopsData(b,'pvp'), campo: 'pvp' },
    valuation:   { label: '📊 TOPs Valuation',   cls: 'val', sort: (a,b) => getTopsData(a,'pl')  - getTopsData(b,'pl'),  campo: 'pl'  },
  };

  const cfg = configs[modoTop];
  if (!cfg) return;

  const btn = document.getElementById('btnVoltarPadrao');
  if (btn) btn.style.display = 'block';

  const totalLabel = document.querySelector('.stat-card .stat-label');
  const totalValue = document.getElementById('totalStocks');
  if (totalLabel) totalLabel.textContent = cfg.label.replace(/\S+\s/, '');
  const cores = { div: '#10b981', pat: '#a78bfa', val: '#f59e0b' };

  /* PATCH 3: filtrar pelo nível de desconto ativo */
  const manual = getManualLevels();
  const l0from = manual[0]?.from ?? 8.3;
  const l1from = manual[1]?.from ?? 16.6;
  const temMod = hasModerato();

  let filtered = allProcessedStocks
    .filter(s => {
      const d = s.discount;
      if (d <= 0) return false;
    ...
    .filter(s => {
      const pl = s.fundamentals?.priceEarnings;
      return !(pl !== null && pl !== undefined && pl <= 0);
    })
    .sort(cfg.sort);

  /* Atualizar contador no stat card com total filtrado */
  if (totalValue) {
    totalValue.textContent = filtered.length;
    totalValue.style.color = cores[cfg.cls];
  }
...
window.closeTopMode = closeTopMode;
...
/* ── PATCH 2: filterAndRender redireciona para renderTopMode se modoTop ativo ── */
...
/* ── GRÁFICO ── */
const _graficoState = {}; // { [index]: { periodo, raf, aberto } }
...
function _gerarDados(anos, preco, media) {
  ...
  for (let i = 0; i < pts; i++) {
    ...
    const alvo = p < 0.55
      ? media * (1.1 - p * 0.16)
      : preco + (media * 0.97 - preco) * (1 - (p - 0.55) / 0.45);
...
function _desenharGrafico(index, anos, preco, media) {
  ...
  const min = Math.min(...dados, media) * 0.962;
  const max = Math.max(...dados, media) * 1.038;
  const toX = i => (i / (dados.length - 1)) * W;
  const toY = v => H - ((v - min) / (max - min)) * H * 0.84 - H * 0.06;
...
window.toggleGrafico    = toggleGrafico;
window.setGraficoPeriodo = setGraficoPeriodo;




/* ── MODAL FULLSCREEN GRÁFICO ── */
...
function abrirModalGrafico(index) {
  const stock = allProcessedStocks.find((s, i) => i === index);
  if (!stock) return;

  _modalStock   = stock;
  _modalPeriodo = discountConfig.periodo;
  _modalDados   = _gerarDados(_modalPeriodo, stock.price, stock.target);
  ...
  const descPct = (((stock.target - stock.price) / stock.target) * 100).toFixed(1);
  document.getElementById('modalDesc').textContent   = `▼ ${descPct}% da média`;
  ...
  document.getElementById('mstatDesconto').textContent = `${descPct}%`;
  const change = stock.change ?? 0;
  const mstatChange = document.getElementById('mstatChange');
  mstatChange.textContent = `${change >= 0 ? '▲' : '▼'} ${Math.abs(change).toFixed(2)}%`;
  mstatChange.className   = `mstat-value ${change >= 0 ? 'pos' : 'neg'}`;

  // sincroniza botões de período
  document.querySelectorAll('#modalPeriodoRow .gpbtn').forEach(b => {
    b.classList.toggle('sel', parseInt(b.textContent) === _modalPeriodo);
  });

  // abre modal
  const overlay = document.getElementById('modalGraficoOverlay');
  const box     = document.getElementById('modalGraficoBox');
...
function fecharModalGrafico(e) {
  if (e && e.target !== document.getElementById('modalGraficoOverlay')) return;
  const box = document.getElementById('modalGraficoBox');
  box.classList.remove('aberto');
  setTimeout(() => {
    document.getElementById('modalGraficoOverlay').classList.remove('visible');
  }, 400);
  cancelAnimationFrame(_rafFull);
}

function _desenharFull() {
  cancelAnimationFrame(_rafFull);
  const stock  = _modalStock;
  const dados  = _modalDados;
  ...
  const min = Math.min(...dados, stock.target) * 0.958;
  const max = Math.max(...dados, stock.target) * 1.042;
  const toX = i => (i / (dados.length - 1)) * W;
  const toY = v => H - ((v - min) / (max - min)) * H * 0.84 - H * 0.06;
  const descPct = (((stock.target - stock.price) / stock.target) * 100).toFixed(1);

  // guarda helpers pro tooltip
  canvas._toX   = toX;
  canvas._toY   = toY;
  canvas._dados = dados;
  ...
  function draw() {
    ...
    ctx.fillText(`Média  ${formatBRL(stock.target)}`, 6, mY - 6);
    ...
    ctx.fillStyle = '#3b82f6'; ctx.fill();
    ctx.beginPath(); ctx.arc(lx, ly, 7 + pulse, 0, Math.PI * 2);
    ctx.strokeStyle = 'rgba(59,130,246,0.25)'; ctx.lineWidth = 1.5; ctx.stroke();

    if (prog > 0.88) {
      const alpha = Math.min(1, (prog - 0.88) / 0.12);
      ctx.font = 'bold 10.5px -apple-system,sans-serif';
      ctx.fillStyle = `rgba(59,130,246,${0.7 * alpha})`;
      ctx.fillText(`▼ ${descPct}%`, lx - 30, (mY + ly) / 2 + 4);
    }

    prog = Math.min(1, prog + 0.014);
    _rafFull = requestAnimationFrame(draw);
  }
  draw();
}

function _setupModalTooltip() {
  const canvas  = document.getElementById('grafFull');
  const tooltip = document.getElementById('modalTooltip');

  function onMove(clientX) {
    const { _toX, _toY, _dados } = canvas;
    if (!_dados) return;
    const rect = canvas.getBoundingClientRect();
    const x    = clientX - rect.left;
    const idx  = Math.round((x / rect.width) * (_dados.length - 1));
    if (idx < 0 || idx >= _dados.length) return;

    const val  = _dados[idx];
    const px   = _toX(idx);
    const py   = _toY(val);
    const tipW = 100;
    let left   = px + 12;
    if (left + tipW > rect.width) left = px - tipW - 12;

    tooltip.style.left = left + 'px';
    tooltip.style.top  = Math.max(4, py - 40) + 'px';
    tooltip.classList.add('visible');

    const mesesAtras = _dados.length - 1 - idx;
    document.getElementById('modalTooltipVal').textContent  = formatBRL(val);
    document.getElementById('modalTooltipData').textContent =
      mesesAtras === 0 ? 'Hoje' : `${mesesAtras} ${mesesAtras === 1 ? 'mês' : 'meses'} atrás`;
  }
  ...
  canvas.addEventListener('touchend',   () => setTimeout(() => tooltip.classList.remove('visible'), 800));
}

function setModalPeriodo(anos, el) {
  _modalPeriodo = anos;
  _modalDados   = _gerarDados(anos, _modalStock.price, _modalStock.target);
  document.querySelectorAll('#modalPeriodoRow .gpbtn')
