...
function analyzeHealth(fundamentals) {
  ...
  items.push({
    label: 'P/L',
    value: (pe !== undefined && pe !== null && pe > 0) ? pe.toFixed(2) : 'N/A',
    status: (pe > 0 && pe < 20) ? 'positive' : (pe >= 20 && pe <= 30) ? 'warning' : 'negative'
  });

  const pvp = fundamentals.priceToBook;
  items.push({
    label: 'P/VP',
    value: (pvp !== undefined && pvp !== null) ? pvp.toFixed(2) : 'N/A',
  ...
  items.push({
    ...
    value: (dy !== undefined && dy !== null && dy > 0) ? `${dy.toFixed(2)}%` : 'N/A',
    status: (dy >= 4) ? 'positive' : (dy > 0 && dy < 4) ? 'warning' : 'negative'
  });
